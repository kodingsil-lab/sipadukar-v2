...
class Filters extends BaseFilters
{
    public function __construct()
    {
        ...
        if ($this->shouldDisableForceHttps()) {
            ...
            $this->globals['before'] = array_values(array_filter(
                $this->globals['before'],
                static fn ($filter): bool => $filter !== 'forcehttps'
            ));
        }
    }
    ...
    public array $globals = [
        'before' => [
            'forcehttps', // Pastikan ini aktif
            'csrf', // Tambahkan ini
            // 'invalidchars',
        ],
        'after' => [
            'secureheaders', // Aktifkan
            'toolbar' => ['except' => ['', '/', 'login', 'portal', 'portal/*', 'public', 'public/*']],
        ],
    ];
    ...
    private function shouldDisableForceHttps(): bool
    {
        $configuredBaseUrl = (string) env('app.baseURL', config(App::class)->baseURL ?? '');
